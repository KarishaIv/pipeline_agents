...
Значения **`agent`** должны совпадать с тем, что записал `prep_rag` в метаданные: `macroeconomy`, `banks`, `currency`, `real_estate` 
---
...
```bash
python -m rag.onnx_quant_bench_e5 --model intfloat/multilingual-e5-large --iters 20 --batches 1 8 32 64
```
