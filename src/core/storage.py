...
class StorageManager:
    ...
    async def save_result_stream(result: Dict, out_dir: Path, run_id: str):
        ...
        summary = {
            ...
            'final_decision': decision_payload.get('will_take_credit'),
            'decision_mode': result.get('decision_mode') or decision_payload.get('decision_mode'),
    ...
    async def save_survey_results(result: Dict, out_dir: Path, run_id: str, survey_questions: List[str]):
        ...
        summary = {
            ...
            'questions_by_index': [
                {'index': i, 'question': q, 'agreement': question_agreement.get(q, 'UNKNOWN')}
