...
def parse_arguments():
    ...
    parser.add_argument('--api_key', type=str, default=os.getenv('YANDEX_API_KEY') or os.getenv('OPENAI_API_KEY'),
                        help='Yandex GPT API key (preferred: YANDEX_API_KEY in .env)')
    ...
    parser.add_argument('--news-context-path', type=str, default=None,
                        help='Optional path to a news-context JSON snapshot for structured survey runtime')
    parser.add_argument('--visualize-sample', type=int, default=5,
                        help='Number of per-person emotion charts to save in credit mode. 0 disables.')
    parser.add_argument('--no-summary-visualize', dest='summary_visualize', action='store_false',
                        help='Disable summary emotions chart in credit mode')
...
def main():
    ...
    api_key = args.api_key or os.getenv('YANDEX_API_KEY') or os.getenv('OPENAI_API_KEY')
    folder_id = args.folder_id or os.getenv('YANDEX_FOLDER_ID')
    if not api_key:
        raise SystemExit('API key not provided. Use --api_key or set YANDEX_API_KEY (preferred) or OPENAI_API_KEY in .env.')
